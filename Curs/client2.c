...
int main(int argc, char *argv[])
{
    ...
    unsigned short TCPServPort = 32001;     /* Server port */
    ...
    for(;;)
    {
        ...
        int bytesRcvd;
        if ((bytesRcvd = recv(TCPsock, &T, sizeof(int), 0)) <= 0)
            DieWithError("recv() failed or connection closed prematurely");
        printf("Received slepping time: %d\n", T);

        printf("Received msg from first client: ");
